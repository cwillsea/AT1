...
export default async function ImportsPage() {
  ...
  const pendingDeposits = deposits.filter(
    (d) => !manuallyPosted.has(d.id) && !deletedSet.has(d.id)
  );
  const totalNet = pendingDeposits.reduce((s, d) => s + d.payoutAmount, 0);
  const totalItems = pendingDeposits.reduce((s, d) => s + d.itemCount, 0);
  const anyUnmatched = pendingDeposits.some((d) => d.rows.some((r) => r.accountNumber === 0));
    ...
    <>
      <div className="flex items-end justify-between px-8 pt-6 pb-5 border-b border-line">
        ...
        <div className="flex gap-4 items-center">
          <GenerateImportButton blockedByUnmatched={anyUnmatched} />
          ...
          <div className="text-right">
            ...
            <div className="font-display text-[18px] text-ink mt-0.5 font-medium">
              {pendingDeposits.length} deposits · {totalItems} items
            </div>
          </div>
          <div className="w-px h-8 bg-line" />
          <div className="text-right">
